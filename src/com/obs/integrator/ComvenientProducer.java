...
public class ComvenientProducer implements Runnable {
	...
	private void setDataForEntitlement(EntitlementsData entitlement) {
		...
		try {
			if (entitlement.getProvisioingSystem().equalsIgnoreCase("Comvenient"))
			 {					
				...
				logger.info(no +") id= "+ entitlement.getId()+" , ServiceId = "+ entitlement.getServiceId() +" , product/Message = "+"'"+ entitlement.getProduct()+"'"+" , setupboxid/SerialNo ="
